...
class AnalysisComparison(AnalysisBase):
	...
	def define_parameters(self, parser):
		...
		parser.add_argument('sim_dir2', nargs='?',
			help=f'''The simulation root directory to compare against. The
				argument can name a subdirectory of "out/" (optionally starting
				with "out/") or an absolute path. It defaults to sim_dir +
				"{constants.OPERON_SUFFIX}".''')
	...
	def run(self, args):
		...
		task = AnalysisComparisonTask(
			input_directory1=args.sim_path,
			input_directory2=args.sim_path2,
