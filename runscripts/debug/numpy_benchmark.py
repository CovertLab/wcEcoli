...
def t3(): # Singular Value Decomposition (SVD)
  N = 3
  t = time()
  for i in range(N):
...
def t4(): # Cholesky Decomposition
  N = 3
  t = time()
  for i in range(N):
      np.linalg.cholesky(F)
  delta = time() - t
  print("Cholesky decomposition of a %dx%d matrix in %0.2f ms." % (size / 2, size / 2, 1e3 * delta / N))


def t5(): # Eigendecomposition
  N = 3
  t = time()
  for i in range(N):
      np.linalg.eig(G)
