...
canvas_obj = VoronoiPlotMain.POLYGON(canvas);
...
index1 = np.array([THE SECOND LAYER HIEARCHY OF EACH POINT: E.G. HOUSEKEEPING, METABOLISM,...]);x
...
class PolygonClass(object):
    def __init__(self, xy):
        ...
        def reorder_points(corners):
            ...
            ordered_points = ordered_points[np.argsort(np.angle((ordered_points - com)[:, 0] 
                + (ordered_points - com)[:, 1]*1j))]
    ...
    def random_points_in_canvas(self, n_points):
        ...
        sites = sites + np.tile(self.xy[0, :], (n_points, 1));
        return sites
    ...
    def point_is_within_canvas(self, P):
        ...
        if point_is_on_corner_of_canvas(self, P):
            ...
            sum_angle = 0;
            for i in range(self.n_corners):
                edge_canvas = self.edges[i];
                a = edge_canvas - P;
                sum_angle += vm.angle(a[0, :], a[1, :]);
    ...
    def find_min_dist_to_border(self, site):
        ...
        def find_min_dist_to_edge(site, edge):
            ...
            if vm.is_on_segment([xp, yp], edge):
                min_dist = dist_perp;
            else:
                min_dist = min(math.sqrt((x1 - xs)**2 + (y1 - ys)**2), math.sqrt((x2 - xs)**2 + (y2 - ys)**2));
            return min_dist
...
class VoronoiClass(object):
    ...
    def find_sites_causing_displacement(self):
        """
        If a site need to expand its area greatly in the next round, the other sites surrounding
        it should be displaced a little bit in order to facilitate the process of optimizing the 
        voronoi diagram. This is part of the speed-up heuristic.
        """
        ...
        try:
            ...
            self.site_causing_disp = 0; #rescue;
    ...
    def adapt_positions_weights(self):
        ...
        for i in range(self.n_sites):
            if not self.polygons[i]: #rescue the sites
                ...
                self.sites[i, :] = (self.polygons[i].xy).mean(axis = 0); #adapt the position to the centroid

        #2. speed up heuristic: if a site is expected to expand its area greatly, its neighbor 
        #should be displaced in advance in order to speed up the optimization process. 
        for i in range(self.n_sites):
            if self.polygons[i]:
                if self.move_me[i] == True: #speed-up heuristic#
                    dist_vec = self.sites[i, :] - self.sites[K, :];
...
class LineClass(object):
    ...
    def find_intersect_with_canvas(self, canvas_obj):
        ...
        def line_intersect_with_edge(self, edge_canvas):
            ...
            DetX = C*self.E - self.F*B; 
            DetY = A*self.F - C*self.D; 
            ...
            if Det == 0:
                ...
                x_int = DetX/Det;
                y_int = DetY/Det;
                if vm.is_on_segment([x_int, y_int], edge_canvas):
                    result = True;
        ...
        for i in range(n_corners):
            ...
            result, intersect_point = self.line_intersect_with_edge(edge_canvas);
...
class RayClass(object):
    ...
    def is_on_ray(self, P):
        ...
        if math.isnan(test):
            ...
            if (int(test*(10**9))/(10.**9) == 0):
                if t_x != 0:
                    if int((P[0] - self.origin[0])/t_x*(10**9))/(10.**9) >= 0:
                        result = True;
                    else:
                        result = False;
                else:
                    if int((P[1] - self.origin[1])/t_y*(10**9))/(10.**9) >= 0:  
                        result = True;
                    else:
                        result = False;
            else:
    ...
    def ray_intersect_with_edge(self, edge_canvas):
        ...
        DetX = C*self.b_r - B*(-self.c_r); 
        DetY = A*(-self.c_r) - C*self.a_r;
        if Det == 0:
            if ((DetX != 0) | (DetY != 0)):
                result = False;
                intersect_point = [];
            else: #(Det == 0) & (DetX == 0) & (DetY == 0)
                P1_is_on_ray = self.is_on_ray(P1); P2_is_on_ray = self.is_on_ray(P2);
                origin_is_on_seg = vm.is_on_segment(self.origin, edge_canvas);
                if P1_is_on_ray & P2_is_on_ray:
                    result = True;
                    intersect_point = [np.inf];
            ...
            y_int = DetY/Det;
            if vm.is_on_segment([x_int, y_int], edge_canvas):
                if self.is_on_ray(np.array([x_int, y_int])):
                    result = True;
                    intersect_point = np.array([x_int, y_int]);
                else:
                    result = False; intersect_point = [];
            else:
                result = False;
                intersect_point = []; 
    ...
    def find_ray_intersect_with_canvas(self, canvas_obj):
        """
        This function finc the point where a ray object is intersected with canvas by checking the 
        intersection with every edge of the canvas.
        """
        intersect_TF_list = [];
        intersect_coordinates = [];
...
class VoronoiMaster():
    ...
    def layered_voronoi(self, df, canvas, i_max, err_thres):
        """
    ...
    def voronoi_treemap(self, canvas_obj, points, values, i_max, err_thres):
        """
        ...
        Certain amount of error is intrinsic to this plot. Therefore, blindly increasing the number of 
        maximum iteration does not guarantee a decrease in error.
        """
        n_points = len(points);
    ...
    def voronoi_treemap_recal(self, voronoi, i_max, err_thres):
        """
    ...
    def recompute_power_diagram(self, voronoi):
        ...
        """
        sites = voronoi.sites; weights = voronoi.weights; 
    ...
    def divide_polygons(self, sites, canvas_obj, args):
        ...
        if n_sites == 2:
            ...
            #1. calculate the edge created by each ray by finding the intersection point of each ray 
            #with canvas
            for i in range(3):
                ray_obj_all[i].find_ray_intersect_with_canvas(canvas_obj);
            
            #2. determine if each site & each corner is above or below each ray by calculating 
            # Ax+By+C >0 or <0 (AB table = above/below table)
            ...
            #1. for all the intersection points, we first determine whether each site & each corners are 
            #above or below each ray.
    ...
    def find_bisector_positive_ray(self, sites, weights, site_label, canvas_obj):
        ...
        if self.point_is_within_triangle(ipoint, sites):
            ...
            #defined in a way that can divide the canvas into 3 polygons and each polygon contains 1 site.
    ...
    def is_on_segment(self, P, edge):
        ...
        if math.sqrt(A**2 + B**2) == 0:
            ...
            if int(test*(10**9))/(10.**9) == 0:
                ...
                x1 = int(x1*(10**9) + 0.5)/(10.**9); x2 = int(x2*(10**9) + 0.5)/(10.**9);
                y = int(y*(10**9) + 0.5)/(10.**9); 
                y1 = int(y1*(10**9) + 0.5)/(10.**9); y2 = int(y2*(10**9) + 0.5)/(10.**9);
...
if __name__ == "__main__":
    ...
    err_thres = 1E-6;
    #create dataframe
    IDs = [randomString(3) for _ in range(160)]
    index1 = ['P1']*32 + ['P2']*32 + ['P3']*32 + ['M1']*32 + ['M2']*32;
    index0 = ['protein']*96 + ['mRNA']*64;
