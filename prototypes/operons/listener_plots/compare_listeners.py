...
def run_listeners(sim1, sim2, outdir, plotoptions):
    ...
    out_file = os.path.join(outdir, 'listener_plots_per_gen.pdf')
    plot_listeners(sim1_data, sim2_data, listener_dict[plotoptions], out_file)


def make_sim_paths(dirname, seednum, dir_type):
    seed = '000000'
    ...
    if dir_type == 0:
        #construct directory name for sim_dir
        ...
        if dirname.count('/') < 1: # assume it is a sim name not a directory and is int out/name
            simdir = os.path.join(str(Path(os.getcwd()).parents[1]), 'wcEcoli', 'out', dirname, seed_path)
        else:
            simdir = os.path.join(str(Path(os.getcwd()).parents[1]), 'wcEcoli', dirname, seed_path)
    return simdir
...
if __name__ == "__main__":
    ...
    sim1dir = make_sim_paths(args['sim1Dir'], args['s1'], 0)
    sim2dir = make_sim_paths(args['sim2Dir'], args['s2'], 1)
