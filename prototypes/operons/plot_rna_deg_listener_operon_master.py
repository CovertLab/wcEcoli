...
def cat_gen_data(gen_path):
    ...
    for idx, simdir in enumerate(sorted(glob.glob(gen_path + '/generation*'))):
        ...
        try:
            ...
            listener_data = TableReader(os.path.join(simOutDir, "RnaDegradationListener"))
...
def plot_operon_master(operon_data, master_data):
    ...
    plot_list = ['fragmentBasesDigested',
    ...
    with PdfPages('out/operon_vs_master_rna_deg_multigen.pdf') as pdf:
        for p in plot_list:
            op_t = 0
            m_t = 0
