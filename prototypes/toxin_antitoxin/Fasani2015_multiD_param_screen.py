...
range_dict = {
				...
				'sigma' : np.linspace(1,20,N),
				'alpha' : np.linspace(1,5,N),
				'kh' : np.logspace(-2, 4, N),
				'kp1' : np.logspace(-2, 4, N),
				'kp2' : np.logspace(-2, 4, N),
				'kt' : np.logspace(-2, 3, N),
				'p' : np.linspace(1, 5, N),
				'n' : np.linspace(0.1, 1.5, N),
				'umax' : np.linspace(3, .01, 10)}
					...
					for n_lambdaT in range_dict['lambda_T']:
						...
						for n_kt in range_dict['kt']:
							...
							for n_n in range_dict['n']:
								...
								if sim_difference > (0.3 * len(Tdat['T1'])):
									setnum += 1
