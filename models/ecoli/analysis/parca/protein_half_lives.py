	...
	def do_plot(self, input_dir, plot_out_dir, plot_out_filename, sim_data_file, validation_data_file, metadata):
		...
		with open(sim_data_file, 'rb') as f:
			...
			with open(os.path.join(plot_out_dir, plot_out_filename + '.tsv'), 'w') as f:
				...
				writer.writerow([
					'monomer_id', 'common_name', 'degradation_rate(1/s)',
					'half_life_(min)', 'degradation_rate_source',
					'protease_assignment', 'ClpP_fraction', 'Lon_fraction', 'HslV_fraction'
				])

				for i, monomer in enumerate(protein_ids):
					writer.writerow([
						protein_ids[i][:-3], common_name[i], degradation_rates[i],
						half_lives[i], deg_rate_source[i], protease_assignment[i],
