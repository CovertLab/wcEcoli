...
class Plot(parcaAnalysisPlot.ParcaAnalysisPlot):
	def do_plot(self, input_dir, plot_out_dir, plot_out_filename, sim_data_file, validation_data_file, metadata):
		...
		with open(sim_data_file, 'rb') as f:
			...
			common_name = sim_data.process.translation.monomer_data['common_name']
			...
			with open(os.path.join(plot_out_dir, plot_out_filename + '.tsv'), 'w') as f:
				...
				writer.writerow([
					'common_name', 'half_life(min)', 'degradation_rate(1/s)',
					'monomer_id', 'degradation_rate_source', 'protease_assignment',
				...
				for i, monomer in enumerate(protein_ids):
					writer.writerow([
						common_name[i], half_lives[i], degradation_rates[i],
						protein_ids[i][:-3], deg_rate_source[i], protease_assignment[i],
						clpp_contribution[i], lon_contribution[i], hslv_contribution[i],
						unexplained_contribution[i]
					])
