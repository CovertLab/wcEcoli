...
def main(seedOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile = None, metadata = None):
	...
	for gen, simDir in enumerate(firstCellLineage):
		...
		rrn16s_fit_init_prob = sim_data.process.transcription.rnaSynthProb[nutrient_condition][idx_16s].sum()
		rrn23s_fit_init_prob = sim_data.process.transcription.rnaSynthProb[nutrient_condition][idx_23s].sum()
		rrn5s_fit_init_prob = sim_data.process.transcription.rnaSynthProb[nutrient_condition][idx_5s].sum()
