...
HIGHLIGHT_IN_PURPLE = ['ADHP-MONOMER[c]', 'G6988-MONOMER[c]', 'EG11111-MONOMER[c]', 'PD03867[c]',
                       'EG50004-MONOMER[c]']  # ['G6890-MONOMER[c]','PD03938[c]','G6737-MONOMER[c]','RPOD-MONOMER[c]','PD02936[c]','RED-THIOREDOXIN2-MONOMER[c]']
...
class Plot(multigenAnalysisPlot.MultigenAnalysisPlot):
    def do_plot(self, seedOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile,
                metadata):
        ...
        complexation_complex_info = [complexation_data[protein_id] for protein_id in protein_ids]
        equilibrium_complex_info = [equilibrium_data[protein_id] for protein_id in protein_ids]
        hi = 5
        ...
        for i in range(
            ...
            diluted_counts_over_time[end_gen,
        ...
        fig.add_trace(go.Scatter(
            ...
            y=log_avg_loss_rate,
            mode='markers',
            customdata=np.stack((monomerIds, half_lives, deg_rate_source, avg_FMC, common_names),
                                axis=-1),
            hovertemplate=
        ...
        if len(HIGHLIGHT_IN_RED) > 0:
            ...
            fig.add_trace(go.Scatter(
                ...
                mode='markers',
                customdata=np.stack(
                    (monomerIds[red_protein_indices], half_lives[red_protein_indices],
                     deg_rate_source[red_protein_indices], avg_FMC[red_protein_indices],
                     common_names[red_protein_indices]), axis=-1),
                hovertemplate=
                "Monomer ID: %{customdata[0]}<br>" +
                "half life: %{customdata[1]}<br>" +
                "source: %{customdata[2]}<br>" +
