...
class Plot(multigenAnalysisPlot.MultigenAnalysisPlot):
	def do_plot(self, seedOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		# Get rna_ids attribute from RnapData table in refernece cell path
		rna_synth_prob_reader = TableReader(
		...
		def plot_ax(ax, y, ylabel, ylim, yticks):
			ax.plot(time / 60, y, color='#8c8c8c')
		...
		# Plot transcription probabilities per copy
		...
		# Plot number of initiation events
		ax3 = plt.subplot(9, 1, 3, sharex=ax1)
		plot_ax(
			ax3, rrna_init_events, 'rRNA\ninitiations',
			[0, 30], [0, 15, 30])

		# Plot counts of active ribosomes
		ax4 = plt.subplot(9, 1, 4, sharex=ax1)
		plot_ax(
			ax4, active_ribosome_counts, '# of active\nribosomes',
			[0, 40000], [0, 40000])

		# Plot number of initiation events
		ax5 = plt.subplot(9, 1, 5, sharex=ax1)
		plot_ax(
			ax5, active_ribosome_counts, '# of active\nribosomes',
			[0, 40000], [0, 40000])

		# Plot number of translated AAs
		ax6 = plt.subplot(9, 1, 6, sharex=ax1)
		plot_ax(
			ax6, translated_aas, '# of translated\naas',
			[0, 500000], [0, 500000])

		# Plot ratio of uncharged tRNAs to all tRNAs
		ax7 = plt.subplot(9, 1, 7, sharex=ax1)
		plot_ax(
			ax7, uncharged_trna_ratio, 'uncharged\ntRNA ratio',
			[0, 0.2], [0, 0.2])

		# Plot ppGpp concentrations
		ax8 = plt.subplot(9, 1, 8, sharex=ax1)
		plot_ax(
			ax8, ppgpp_conc, 'ppGpp\nconc (uM)',
			[0, 200], [0, 100, 200])

		# Plot doubling times
		ax9 = plt.subplot(9, 1, 9, sharex=ax1)
		plot_ax(
			ax9, doubling_times, 'DT\n(min)',
			[0, 100], [0, 50, 100])
		ax9.get_xaxis().set_visible(True)
		ax9.spines["bottom"].set_visible(True)
		ax9.set_xlabel('Time (generations)')
		ax9.set_xticks((gen_start_times / 60).tolist() + [time[-1] / 60])
		ax9.set_xticklabels(np.arange(len(gen_start_times) + 1).tolist())
