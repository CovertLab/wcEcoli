...
new_gene_internal_shift_with_kcat variant.

Adapted from new_gene_translation_efficiency_heatmaps.py to handle the
wildtype control variant (index 21, no kcat constraints) and to fall back
to imported constants when metadata keys are absent.
...
# Wildtype control variant index (no kcat constraints, no new gene grid position).
_WILDTYPE_INDEX = 21
...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	...
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile,
		...
		# Fall back to imported constants when metadata keys are absent
		# (e.g. when running on new_gene_internal_shift_with_kcat sims).
		if ('new_gene_expression_factors' in metadata and
			...
			new_gene_expression_factors = list(NEW_GENE_EXPRESSION_FACTORS)
			new_gene_translation_efficiency_values = list(
				NEW_GENE_TRANSLATION_EFFICIENCY_VALUES)
		...
		for index in variants:
			# Skip wildtype control (no new gene grid position)
