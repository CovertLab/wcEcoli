...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		completion_rates = np.zeros(len(variant_indexes))
		...
		for i, variant_index in enumerate(variant_indexes):
			...
			completion_rates[i] = num_last_gen / num_zero_gen
		...
		plt.ylabel(f'% of Seeds\nCompleting {n_total_gens} Gens', fontsize=8)
		...
		plt.bar(
			[str(w) for w in kinetic_weights],
			avg_homeostatic_obj_values / (avg_homeostatic_obj_values * kinetic_weights),
		)
		plt.ylabel('Ratio of Homeostatic\nto Kinetic\nObjective Value', fontsize=8)
		plt.xlabel('Kinetic Objective Weight')
