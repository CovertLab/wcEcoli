"""
Plot a scatterplot of the average new gene protein counts vs. active
ribosome counts. This plot is intended to be run on simulations where
...
from models.ecoli.analysis.variant.avg_ppgpp_conc_vs_doubling_time import INTERACTIVE_PLOT
...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile,
		...
		ribosome_count = np.zeros(len(variants))
		...
		for i, variant in enumerate(variants):
			...
			ribosome_count[i] = np.mean(ribosome_counts)

		plt.figure()
		plt.xlabel("Number of Active Ribosomes")
		plt.ylabel("Protein Counts")
		plt.ylim(0, 1.2e6)
		plt.scatter(
			ribosome_count[plot_variant_mask],
			avg_ng_monomer[plot_variant_mask])
		plt.plot(
			ribosome_count[plot_variant_mask],
			np.poly1d(np.polyfit(ribosome_count[plot_variant_mask],
		...
		if INTERACTIVE_PLOT:
			...
			fig = px.scatter(
				x=ribosome_count[plot_variant_mask],
				y=avg_ng_monomer[plot_variant_mask],
				hover_name=np.array(variants)[plot_variant_mask],
				labels={'x': 'Number of Active Ribosomes', 'y': 'Protein Counts'},
				hover_data={
