...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		for i, variant in enumerate(variants):
			...
			lambdas[i] = sim_data.process.metabolism.kinetic_objective_weight
			...
			# Growth rates
			growth_rates[i] = np.mean(growth_rate)

			# Metabolite comparison
			actual_conc = np.mean(actual_conc, axis=0)
			target_conc = np.mean(target_conc, axis=0)
			conc_correlation[i] = np.corrcoef(actual_conc, target_conc)[0, 1]
			n_conc_off_axis[i] = np.sum(np.abs((target_conc - actual_conc)/target_conc) > FRAC_CONC_OFF_AXIS)
			...
			flux_correlation[i] = np.corrcoef(actual_flux, target_flux)[0, 1]
			n_flux_off_axis[i] = np.sum(np.abs((target_flux - actual_flux)/target_flux) > FRAC_FLUX_OFF_AXIS)
			mask = (actual_flux != 0)
			nonzero_flux_correlation[i] = np.corrcoef(actual_flux[mask], target_flux[mask])[0, 1]
			n_flux_above_0[i] = np.sum(actual_flux > 0) + np.sum((actual_flux == 0) & (target_flux == 0))

			# Toya comparison
			ave_toya_model = np.array([np.mean(toya_model_fluxes[rxn]) for rxn in toya_reactions])
			correlation_coefficient[i] = np.corrcoef(ave_toya_model, toya_fluxes)[0, 1]
			filtered_correlation_coefficient[i] = np.corrcoef(ave_toya_model[outlier_filter], toya_fluxes[outlier_filter])[0, 1]
			...
			kinetic_objective_value[i] = np.mean(kinetic_objective)
			kinetic_objective_std[i] = np.std(kinetic_objective)
			homeostatic_objective_value[i] = np.mean(homeostatic_objective_values)
			homeostatic_objective_std[i] = np.std(homeostatic_objective_values)
