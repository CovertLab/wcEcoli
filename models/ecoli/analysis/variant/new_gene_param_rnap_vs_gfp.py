...
RNA polymerase counts. This plot is intended to be run on simulations where
...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile,
		...
		active_rnap = np.zeros(len(variants))
		...
		for i, variant in enumerate(variants):
			...
			active_rnap[i] = np.mean(active_rnap_counts)

		plt.figure()
		plt.xlabel("Active RNAP")
		plt.ylabel("Protein Counts")
		plt.ylim(0, 1.2e6)
		plt.scatter(active_rnap[plot_variant_mask], avg_ng_monomer[plot_variant_mask])
		plt.plot(
			active_rnap[plot_variant_mask],
			np.poly1d(np.polyfit(active_rnap[plot_variant_mask],
