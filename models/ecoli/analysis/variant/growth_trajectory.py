	...
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		for variant in variants:
			...
			for seed in ap.get_seeds(variant):
				...
				protein_growth_ma = np.convolve(protein_growth, np.ones(200) / 200, mode='valid')
				rna_growth_ma = np.convolve(rna_growth, np.ones(200) / 200, mode='valid')
				small_mol_growth_ma = np.convolve(small_mol_growth, np.ones(200) / 200, mode='valid')
