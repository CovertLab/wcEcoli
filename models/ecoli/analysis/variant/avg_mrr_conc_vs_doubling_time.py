"""
Plot a scatterplot of the average mrr concentration vs. doubling time.
...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile,
		...
		avg_mrr_count = np.zeros(len(variants))
		avg_mrr_conc = np.zeros(len(variants))
		...
		for i, variant in enumerate(variants):
			...
			if variant == min_variant:
				...
				mrr_index = uniqueMoleculeCounts.readAttribute(
					"uniqueMoleculeIds").index('mrr')

			# get average mrr count
			active_mrr_counts = read_stacked_columns(
				all_cells, 'UniqueMoleculeCounts', 'uniqueMoleculeCounts',
				remove_first=True,
				ignore_exception=True)[:, mrr_index]
			avg_mrr_count[i] = np.mean(active_mrr_counts)
			...
			avg_mrr_conc[i] = np.mean(active_mrr_counts * counts_to_molar)

		plt.figure()
		plt.xlabel("mrr Concentration")
		plt.ylabel("Doubling Time (min)")
		plt.scatter(
			avg_mrr_conc[plot_variant_mask],
			doubling_times[plot_variant_mask])
		plt.plot(
			avg_mrr_conc[plot_variant_mask],
			np.poly1d(np.polyfit(avg_mrr_conc[plot_variant_mask],
		...
		if INTERACTIVE_PLOT:
			...
			fig = px.scatter(
				x=mrr_concentration[plot_variant_mask],
				y=doubling_times[plot_variant_mask],
				hover_name=np.array(variants)[plot_variant_mask],
				labels={'x': 'mrr Concentration', 'y': 'Doubling Time (min)'},
				hover_data={
