"""
Summary plot for the new_gene_trl_eff_v2_estimator_sweep variant comparing
doubling-time impacts across three kcat regimes (gfp_only, gap, smoothed_max)
as GFP translation efficiency is swept at a fixed expression factor and a fixed
kcat multiplier of 1.0.  The metabolic stress comes from reduced enzyme counts
as GFP competes for ribosomes.

Adapted from kcat_v2_estimator_sweep_comparison.py.  Produces four panels:

  1. Completion rate per variant (bar; colored by block, lighter shades for
     smaller translation efficiencies).
...
# One base color per block.  Translation efficiency shades lighter -> darker
# toward larger efficiencies (more ribosome competition / stress), so within a
# block the trl_eff = 0 bar is the lightest.
BLOCK_BASE_COLORS = (
	(0.498, 0.498, 0.498),  # gfp_only -- grey   (#7f7f7f)
	(0.122, 0.467, 0.706),  # gap      -- blue   (#1f77b4)
	(0.173, 0.627, 0.173),  # smoothed_max -- green (#2ca02c)
)
WILDTYPE_COLOR = (0.1, 0.1, 0.1)  # near-black
_TRL_EFF_MAX = max(TRL_EFF_VALUES) if TRL_EFF_VALUES else 1.0


def _shade(base_rgb, trl_eff):
	"""Mix base color with white based on translation efficiency.

	trl_eff = max -> 0% white + 100% base (darkest)
	trl_eff = 0   -> 80% white + 20% base (lightest)
	"""
	denom = _TRL_EFF_MAX if _TRL_EFF_MAX > 0 else 1.0
	frac = max(0.0, min(0.8, 0.8 * (1.0 - trl_eff / denom)))
	r, g, b = base_rgb
	return (
		r + frac * (1 - r),
		g + frac * (1 - g),
...
def _variant_color(index):
	...
	return _shade(BLOCK_BASE_COLORS[block_idx], TRL_EFF_VALUES[te_idx])
...
def _variant_label(index):
	if is_wildtype(index):
		return 'WT'
	block_idx, te_idx = variant_to_block(index)
	return f'{BLOCK_SHORT_NAMES[block_idx]}\n{TRL_EFF_VALUES[te_idx]}'
...
class Plot(variantAnalysisPlot.VariantAnalysisPlot):
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile,
		...
		# Legend: one entry per block + wildtype
		legend_elements = [Patch(facecolor=WILDTYPE_COLOR, label='wildtype')]
		for block_idx in range(len(BLOCK_ESTIMATORS)):
			legend_elements.append(Patch(
				facecolor=BLOCK_BASE_COLORS[block_idx],
				label=f'{BLOCK_SHORT_NAMES[block_idx]} '
				f'(darker = higher trl_eff)'))
