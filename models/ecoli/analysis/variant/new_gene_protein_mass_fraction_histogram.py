"""
Plot histogram of new gene protein mass fraction, colored by variant,
 for all generations, early generations, and/or late (i.e. not early) generations
"""
	...
	def do_plot(self, inputDir, plotOutDir, plotOutFileName, simDataFile,
		...
		for variant in variants:
			...
			for i in range(len(new_gene_mRNA_ids)):
				new_gene_monomer_counts[i][variant] = np.log10(
					avg_new_gene_monomer_counts[:,i] + 1)
		...
		for i in range(len(new_gene_monomer_ids)):
			...
			for j in range(len(data_start)):
				...
				labeled_indexable_hist(
					self, axes, new_gene_monomer_mass_fraction[i], generations,
				...
				exportFigure(plt, plotOutDir, plotOutFileName + plot_label[j]
							 + new_gene_monomer_ids[i][:-3], metadata)
		plt.close("all")
