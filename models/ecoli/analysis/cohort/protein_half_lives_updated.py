...
class Plot(parcaAnalysisPlot.ParcaAnalysisPlot):
	def do_plot(self, input_dir, plot_out_dir, plot_out_filename, sim_data_file, validation_data_file, metadata):
		...
		with open(sim_data_file, 'rb') as f:
			...
			# Extract protein half life and protease assignment information:
			...
			clpp_contribution = sim_data.process.translation.monomer_data['ClpP_contribution_fraction']
			lon_contribution = sim_data.process.translation.monomer_data['Lon_contribution_fraction']
			hslv_contribution = sim_data.process.translation.monomer_data['HslV_contribution_fraction']
			unexplained_contribution = sim_data.process.translation.monomer_data['Unexplained_contribution_fraction']
			deg_rate_source = sim_data.process.translation.monomer_data['half_life_source']
			...
			cistron_id = sim_data.process.translation.monomer_data['cistron_id']
			common_names = [sim_data.common_names.get_common_name(id) for id in cistron_id]
			length = sim_data.process.translation.monomer_data['length']
			aa_counts = sim_data.process.translation.monomer_data['aa_counts']
			mw = sim_data.process.translation.monomer_data['mw']



			# yibQ's protein deg rate is multiplied by 10 in adjustments,
			# do not inlcude it in the plot to avoid skewing the visualization by too much
			...
			with open(os.path.join(plot_out_dir, plot_out_filename + '.tsv'), 'w') as f:
				...
				writer.writerow([
					'monomer_id', 'common_name', 'half_life(min)', 'degradation_rate(1/s)',
					'degradation_rate_source', 'protease_assignment',
					'ClpP_contribution', 'Lon_contribution', 'HslV_contribution',
					'Unexplained_contribution', 'cistron_id', 'length', 'aa_counts', 'mw'
				])

				for i, monomer in enumerate(protein_ids):
					writer.writerow([protein_ids[i][:-3], common_names[i], half_lives[i], degradation_rates[i],
						deg_rate_source[i], protease_assignment[i],
						clpp_contribution[i], lon_contribution[i], hslv_contribution[i],
						unexplained_contribution[i], cistron_id[i], length[i], aa_counts[i], mw[i]
					])
