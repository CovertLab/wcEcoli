"""
Plot mRNA and free monomer counts across simulation seeds and generations.
...
See complexed_monomer_counts.py to make plots of the complexed monomer counts,
...
class Plot(cohortAnalysisPlot.CohortAnalysisPlot):
	...
	def extract_data(self, simOutDir, cell_paths, monomer_ids, cistron_ids):
		"""
		Extracts the time, free monomer counts, and mRNA counts data for
		...
		Returns: The time, free monomer counts, and mRNA counts for the
		...
		# Obtain the free monomer counts for each protein
		(monomer_counts,) = read_stacked_bulk_molecules(
			cell_paths, monomer_ids, ignore_exception=True)
		# Obtain the mRNA counts for each protein
	...
	def plot_counts_per_seed(self, seed, cell_paths, monomer_ids, cistron_ids,
		...
		"""
		Plots the free monomer counts and mRNA counts for a given seed.
        ...
        Args:
            ...
            monomer_counts: free monomer counts for proteins of interest
        ...
        Returns: A plot export for the free monomer counts and mRNA counts
		...
		if len(monomer_ids) == 1:
			...
			plt.plot(time / 60., monomer_counts, label=name)
		else:
	...
	def plot_counts_per_protein(self, simOutDir, protein, cistron, seeds,
		...
		"""
        Plot the free monomer counts and mRNA counts for a given protein
        ...
        Returns: A plot of the free protein counts and mRNA counts for the
		...
		for seed in seeds:
			...
			# plot the data
			plt.plot(time / 60., monomer_counts, label=f'seed {seed}',
					 alpha=0.5)
	...
	def do_plot(self, seedOutDir, plotOutDir, plotOutFileName, simDataFile,
		...
		# iterate through the seeds and plot the counts data for each seed
		for seed in seeds:
			# Generate all the cell paths for the seed:
			cell_paths = self.ap.get_cells(seed=[seed], only_successful=True)
			# Extract data for the seed
		...
		# iterate over each individual protein and plot across all seeds:
		for protein in interest_proteins:
