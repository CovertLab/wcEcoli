...
Two cell sets are reported side by side, filled from ONE pass over simOut:

  <name>_all_*        every strict-successful cell (generation >= 8)
  <name>_complete_*   only cells from seeds that reached the final generation

The `_complete` set is a COMPLETE-SEED filter -- reached the final generation --
and NOT the strict successful-seed filter the rest of the suite uses
(sc.compute_seed_success, which also requires that no cell hit the 180-minute
doubling cap). Seeds that die mid-run are a major source of "near-ubiquitous"
absences, since a dying cell drops many genes at once in its last generations,
so comparing the two _definition_summary.tsv files shows how much of the
apparent subgenerational signal is a dying-seed artifact rather than genuine
...
class _Accumulators:
	"""The O(n_genes) running sums for one cell set.

	Two of these are filled from the same pass over simOut -- one for every
	strict-successful cell, one for the complete-seed subset -- so reporting
	both cell sets costs no extra reads.
	"""

	def __init__(self, n_genes):
		self.sum_present = np.zeros(n_genes)
		self.sum_any_init = np.zeros(n_genes)
		self.sum_init_events = np.zeros(n_genes)
		self.sum_any_synth = np.zeros(n_genes)
		self.sum_synth_events = np.zeros(n_genes)
		self.max_mRNA_counts = np.zeros(n_genes, dtype=np.int64)
		self.max_protein_counts = np.zeros(n_genes, dtype=np.int64)
		self.sum_mRNA_counts = np.zeros(n_genes)
		self.sum_protein_counts = np.zeros(n_genes)
		self.total_timesteps = 0
		self.absent_cells_by_gene = {}
		self.disqualified_absent = np.zeros(n_genes, dtype=bool)
		self.included_cells = []

	def add_cell(self, cell_path, mRNA_counts, monomer_counts, init_per_cell,
			present_in_cell, synth_per_cell):
		self.sum_present += present_in_cell                   # def 1
		self.sum_any_init += init_per_cell > 0                # def 2
		self.sum_init_events += init_per_cell                 # def 3
		if synth_per_cell is not None:
			self.sum_any_synth += synth_per_cell > 0          # def 4
			self.sum_synth_events += synth_per_cell           # def 5

		self.max_mRNA_counts = np.maximum(
			self.max_mRNA_counts, mRNA_counts.max(axis=0))
		self.max_protein_counts = np.maximum(
			self.max_protein_counts, monomer_counts.max(axis=0))
		self.sum_mRNA_counts += mRNA_counts.sum(axis=0)
		self.sum_protein_counts += monomer_counts.sum(axis=0)
		self.total_timesteps += mRNA_counts.shape[0]

		# Track absent cells for genes still eligible to be "close to 1".
		for gene_index in np.nonzero(
				~present_in_cell & ~self.disqualified_absent)[0]:
			gene_index = int(gene_index)
			cells = self.absent_cells_by_gene.setdefault(gene_index, [])
			cells.append(cell_path)
			if len(cells) > MAX_ABSENT_CELLS_TO_REPORT:
				self.disqualified_absent[gene_index] = True
				del self.absent_cells_by_gene[gene_index]

		self.included_cells.append(cell_path)
...
class Plot(cohortAnalysisPlot.CohortAnalysisPlot):
	...
	def _def5_ci_summary_rows(self, plotOutDir, n_genes, n_threshold_columns):
		...
		try:
			...
			print('\nNote: no raw extraction in %s, so the canonical def5_CI row is '
				'omitted from the summary. Run subgen_extract.py to include it.'
				% plotOutDir)
			return []
		if clf['n_seeds'] == 0:
			return []
	...
	def _write_cell_set(self, plotOutDir, plotOutFileName, set_name, acc,
			has_synth, n_genes, gene_ids, mRNA_cistron_ids, monomer_ids):
		"""Write the three tables for one cell set ('all' or 'complete')."""
		prefix = os.path.join(plotOutDir, '%s_%s' % (plotOutFileName, set_name))
		n_cells = len(acc.included_cells)
		if n_cells == 0:
			print('\n[%s] No readable cells; skipping this cell set.' % set_name)
			return
		print('\n[%s] Aggregated %d cells.' % (set_name, n_cells))

		# Aggregate accumulators into the five definitions
		p_present = acc.sum_present / n_cells
		p_any_init = acc.sum_any_init / n_cells
		mean_init_events = acc.sum_init_events / n_cells
		if has_synth:
			p_any_synth = acc.sum_any_synth / n_cells
			mean_synth_events = acc.sum_synth_events / n_cells
		else:
		...
		mean_mRNA_counts = acc.sum_mRNA_counts / acc.total_timesteps
		mean_protein_counts = acc.sum_protein_counts / acc.total_timesteps

		# Write the main per-gene table
		main_path = prefix + '.tsv'
		...
		with open(main_path, 'w') as f:
			...
			for i in range(n_genes):
				writer.writerow([
					...
					acc.max_mRNA_counts[i], mean_mRNA_counts[i],
					acc.max_protein_counts[i], mean_protein_counts[i],
					])

		# Definition comparison
		summary_path = prefix + '_definition_summary.tsv'
		...
		print('\n[%s] Subgenerational gene counts (0 < value < threshold; the '
			'def5_CI rows are the canonical classification, not a threshold '
			'sweep):' % set_name)
		print('\t'.join(header))
		for row in summary_rows:
		...
		absent_path = prefix + '_near_ubiquitous_absences.tsv'
		print('Writing %s' % absent_path)
		absent_rows = []
		for gene_index, cells in acc.absent_cells_by_gene.items():
			...
			if acc.sum_present[gene_index] == 0:
				continue
	...
	def do_plot(self, variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		# Which seeds reached the final generation. Every cell is analyzed; this
		# set only decides which of the two accumulators a cell also feeds, so
		# the `_complete` tables come free with the `_all` ones.
		...
		complete_seeds = {
			...
			if last == final_gen}
		print('%d of %d seeds are complete (reached generation %d).'
			% (len(complete_seeds), len(last_gen_by_seed), final_gen))
		...
		# One pass over simOut fills both cell sets.
		acc_all = _Accumulators(n_genes)
		acc_complete = _Accumulators(n_genes)
		skipped_cells = []
		...
		for i, cell_path in enumerate(cell_paths):
			...
			synth_per_cell = synth_events.sum(axis=0) if has_synth else None

			acc_all.add_cell(cell_path, mRNA_counts, monomer_counts,
				init_per_cell, present_in_cell, synth_per_cell)
			if _parse_cell_id(cell_path)[0] in complete_seeds:
				acc_complete.add_cell(cell_path, mRNA_counts, monomer_counts,
					init_per_cell, present_in_cell, synth_per_cell)

		if len(acc_all.included_cells) == 0:
			print('No readable cells found. Skipping.')
			return

		for set_name, acc in (('all', acc_all), ('complete', acc_complete)):
			self._write_cell_set(plotOutDir, plotOutFileName, set_name, acc,
				has_synth, n_genes, gene_ids, mRNA_cistron_ids, monomer_ids)
		...
		run_metadata = {
			...
			'cells': {
				...
				'n_included': len(acc_all.included_cells),
				'n_skipped': len(skipped_cells),
				'included': acc_all.included_cells,
				'skipped': skipped_cells,
				},
			'seeds': {
				'final_generation': final_gen,
				'n_seeds_seen': len(last_gen_by_seed),
				'n_seeds_complete': len(complete_seeds),
				'n_cells_all': len(acc_all.included_cells),
				'n_cells_complete': len(acc_complete.included_cells),
				},
			}
