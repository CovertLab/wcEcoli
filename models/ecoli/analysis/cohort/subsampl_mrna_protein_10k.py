...
class Plot(cohortAnalysisPlot.CohortAnalysisPlot):
    def do_plot(self, variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
        ...
        total_monomer_counts = np.empty((0, len(gene_ids)), dtype=np.float64)
        ...
        monomer_table_cols = ['seed', 'time_step', 'generation_start_time'] + monomer_ids 
        with open(os.path.join(plotOutDir, plotOutFileName + 'corresponding_monomers.tsv'), 'w') as f:
            writer = csv.writer(f, delimiter='\t')
