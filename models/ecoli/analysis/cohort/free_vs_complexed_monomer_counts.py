...
this cohort analysis script identifies monomers with short half lives (can be specified under "half_life_threshold" variable) that also
have assigned proteases. The distributions of these monomers (as fractions of total monomer) are then plotted across the forms in which they
are found in the cell, either as free monomer or in complexed forms.

The script also plots the dynamics of mRNA and monomer counts of a chosen list of monomer ids specified under variable "monomer_complex_short_list".
"""
...
monomer_complex_short_list = ['EG10156[c]', 'EG10159[c]']
...
class Plot(cohortAnalysisPlot.CohortAnalysisPlot):
	def do_plot(self, variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		def monomer_and_cistron_counts(cell_paths,
			...
			cistrons_interest = [
				monomer_id_to_cistron_id[monomer]
				for monomer in total_monomer_dict.keys()
			]

			cistron_indices = np.array([
				cistron_id_to_index[cistron_id] for cistron_id
				in cistrons_interest
			])

			# Get the mRNA counts for each gene/protein
			mRNA_counts = read_stacked_columns(cell_paths, 'RNACounts',
											   'mRNA_cistron_counts',
											   fun=lambda x: x[:, cistron_indices], ignore_exception=True)
			...
			return mRNA_counts, cistrons_interest, distributed_monomer_counts, distributed_monomer_ratio, array_cols

		def get_avg_protein_per_cell(start_generation_indices, end_generation_indices, distributed_monomer_data):
			all_generations_mean = []
			for start, end in zip(start_generation_indices, end_generation_indices):
				generation_timepoints = distributed_monomer_data[start:end, :]
				valid_timepoints = ~np.isnan(generation_timepoints).any(axis=1)
				generation_mean = np.mean(generation_timepoints[valid_timepoints, :], axis=0)
				all_generations_mean.append(generation_mean)
			return np.array(all_generations_mean)
		...
		def plot_monomer_complex_dynamics(distributed_monomer_counts,
		...
		def plot_mRNA_dynamics(mRNA_counts,
							   cistrons_interest,
							   cistrons_short_list,
							   time,
							   end_generation_times,
							   seed):


			# mRNA Counts Plot
			num_groups = len(cistrons_short_list)
			cols = min(num_groups, 4)   # Number of columns for the grid
			rows = (num_groups + cols - 1) // cols  # Calculate the number of rows
			fig_width = cols * 10
			fig_height = fig_width/2
			...
			# Flatten the axes array for easier iteration
			for i, ax in enumerate(axes):
				if i < len(cistrons_short_list):  # Check if there's a cistron for this subplot
					cistron = cistrons_short_list[i]
					cistron_index = [i for i, x in enumerate(cistrons_interest) if x == cistron]
					cistron_counts = mRNA_counts[:, cistron_index]
					ax.plot(time/60, cistron_counts, color='black', label=cistron)
					ax.set_xlabel("Time (min)");
					ax.set_ylabel("mRNA Counts")
					ax.set_title(cistron)
					ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
					for x in end_generation_times/60:
						ax.axvline(x=x,
								   color='grey',
			...
			plt.tight_layout()
			exportFigure(plt, plotOutDir, plotOutFileName + f'_mRNA_dynamics_{seed}', metadata)

		def plot_bar_graphs_per_monomer(seed, mean_monomer_ratio_all_cells, monomers_and_complexes_names, annota_edit_monomer_dict, figsize=(10, 40)):
			...
			for i, (monomer_id, forms) in enumerate(sorted(annota_edit_monomer_dict.items())):
				...
				forms_indeces = [i for i, x in enumerate(monomers_and_complexes_names) if x in forms]
				forms_array = mean_monomer_ratio_all_cells[:, forms_indeces]
				valid_cells = ~np.isnan(forms_array).any(axis=1)
				if not np.all(valid_cells):
					continue
				means = np.mean(forms_array[valid_cells, :], axis=0)  # Calculate the mean across cells for each monomer
				errors = np.std(forms_array[valid_cells, :], axis=0)/np.sqrt(len(forms))  # Calculate standard error
			...
			plt.tight_layout()
			exportFigure(plt, plotOutDir, plotOutFileName + f'_monomer_distribution_{seed}', metadata)
		...
		for seed in self.ap.get_seeds():
			...
			if not np.all([self.ap.get_successful(cell) for cell in cell_paths]):
				continue
			mRNA_counts, cistrons_interest, distributed_monomer_counts, distributed_monomer_ratio, array_cols = monomer_and_cistron_counts(
				cell_paths,
				monomers_to_complexes,
				total_monomer_dict,
				monomers_comp_of_interest,
			...
			if monomer_complex_short_list is not None:
				cistrons_short_list = [
					monomer_id_to_cistron_id[monomer]
				for monomer in monomer_complex_short_list
			]

				plot_monomer_complex_dynamics(distributed_monomer_counts,
										  monomers_and_complexes_names,
										  annota_edit_monomer_dict,
										  monomer_complex_short_list,
										  time, end_generation_times,
										  seed)
				plot_mRNA_dynamics(mRNA_counts,
								   cistrons_interest,
								   cistrons_short_list,
								   time,
								   end_generation_times,
			...
			plot_bar_graphs_per_monomer(seed, mean_ratio,monomers_and_complexes_names, annota_edit_monomer_dict,
															  figsize=(10, 20))
		...
		plot_bar_graphs_per_monomer('all_seeds', mean_monomer_ratio_all_cells, monomers_and_complexes_names, annota_edit_monomer_dict, figsize=(10, 20))
