...
class Plot(cohortAnalysisPlot.CohortAnalysisPlot):
	def generate_data(self, simDataFile, validationDataFile):
		...
		all_cells = self.ap.get_cells(generation=np.arange(IGNORE_FIRST_N_GENS, self.n_total_gens),
									  only_successful=True)
		# Get the protein counts for each gene/protein
		gen_avg_monomer_counts = (
			read_stacked_columns(all_cells, 'MonomerCounts', 'monomerCounts', fun=lambda x: np.mean(x[:], axis=0)))
	...
	def do_plot(self, variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
		...
		outDir = variantDir
		save_pth = os.path.join(outDir, "cohort_saved_protein_count_data")
		...
		log_PC_ids = np.transpose(np.array(self.all_monomer_ids));
		log_PC_ids = log_PC_ids.reshape(-1, 1)
		log_PCs = np.transpose(np.array(log_PCs));
		log_PCs = log_PCs.reshape(-1, 1)
		...
		if save_filtered_data == 1:
			...
			F_PC_ids = np.transpose(np.array(F_PC_ids));
			F_PC_ids = F_PC_ids.reshape(-1, 1)
			F_PCs = np.transpose(np.array(F_PCs));
			F_PCs = F_PCs.reshape(-1, 1)
			...
			F_log_PC_ids = np.transpose(np.array(F_PC_ids));
			F_log_PC_ids = F_log_PC_ids.reshape(-1, 1)
			F_log_PCs = np.transpose(np.array(F_log_PCs));
			F_log_PCs = F_log_PCs.reshape(-1, 1)
		...
		if save_with_validation_data == 1:
			...
			sim_schmidt_counts, val_schmidt_counts, s_ids, sim_wisniewski_counts, val_wisniewski_counts, w_ids = self.get_validation_data(
				variantDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata)
