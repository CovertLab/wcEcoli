"""
v2 distribution analysis for below-line essential reaction-catalyst kcats.

For every (reaction, catalyst) pair that the existing kcat_estimations.py would
analyse, accumulate the distribution of four quantities across all post-burn-in
cells in the cohort:

	* kcat estimate     (flux / enzyme concentration; 1/s)
	* reaction flux     (mmol/g DCW/h)
	* enzyme concentration (M)
	* enzyme counts     (counts)
...
Outputs in plotOutDir (two parallel "views" of the data):
  ...
  Per-view CSVs (always produced):
	* kcat_estimates_v2_summary.csv         -- four rows per pair, one per
	                                           quantity, with n, percentiles,
	                                           min/mean/max, and three "spike"
	                                           ratios (max/p99, max/median,
	                                           max/mean).  Rows sorted by kcat
	                                           max/p99 descending so reactions
	                                           where the max is far above the
	                                           bulk appear first.
	* kcat_estimates_v2_summary_active.csv

  Per-view PDFs (controlled by PLOT_STYLES_ENABLED; rug is on by default):
	* kcat_estimates_v2_dist_logy{,_active}.pdf      one slide-sized 2x2 page
	                                                 per pair, log-y histograms
	* kcat_estimates_v2_dist_ecdf{,_active}.pdf      linear histogram + ECDF
	                                                 on a twin axis
	* kcat_estimates_v2_dist_logy_rug{,_active}.pdf  log-y histogram + rug of
	                                                 top values (DEFAULT)

Toggle MAKE_DISTRIBUTION_PDFS = False to produce only the CSV.  Toggle
MAKE_KCAT_OVERLAY_PDFS = True (with a working _select_kcat) to additionally emit
"<...>_with_estimate.pdf" PDFs that overlay the chosen kcat as a dashed vertical
line on the kcat subplot.

Memory: instead of storing every (T, n_pairs) sample, this script accumulates
log-spaced histograms per pair (20 bins/decade) and tracks running min/max/sum/n
plus the top-K extreme samples for the rug plots.  Resident footprint scales
with n_pairs, not with the number of cells.
"""
...
def _select_kcat(pair_entry):
	"""Return a single kcat estimate (1/s) for this pair, or None.

	Stub for the v2 estimator -- to be implemented in a follow-up.  Receives
	the full per-pair stats dict (including per-quantity percentiles, top-K
	values, etc.) so the estimator can use any of that information.
	"""
	return None
...
class Plot(cohortAnalysisPlot.CohortAnalysisPlot):
	def do_plot(self, variantDir, plotOutDir, plotOutFileName, simDataFile,
		...
		for view in VIEWS:
			...
			csv_path = os.path.join(
				plotOutDir,
				f'kcat_estimates_v2_summary{VIEW_SUFFIX[view]}.csv')
		...
		if MAKE_KCAT_OVERLAY_PDFS:
			for view in VIEWS:
				pair_stats = pair_stats_by_view[view]
				chosen_kcats = {p: _select_kcat(pair_stats[p])
					for p in range(n_pairs)}
				if all(v is None for v in chosen_kcats.values()):
					print(f"MAKE_KCAT_OVERLAY_PDFS=True but _select_kcat returned "
						f"None for every pair in view '{view}'; skipping overlay PDFs.")
					continue
				self._make_pdfs(
					plotOutDir,
					sorted_indices_by_view[view], pair_stats,
					accumulators[view], top_k[view],
					chosen_kcats=chosen_kcats,
					suffix=f'{VIEW_SUFFIX[view]}_with_estimate')
	...
	def _make_pdfs(self, plotOutDir, sorted_indices, pair_stats,
		...
		paths = {
			s: os.path.join(
				plotOutDir, f'kcat_estimates_v2_dist_{s}{suffix}.pdf')
			for s in styles
		}
	...
	def _render_figure(self, entry, accumulators, top_k, p, style, chosen_kcat):
		...
		for q in QUANTITY_ORDER:
			...
			if q == 'kcat' and chosen_kcat is not None and np.isfinite(chosen_kcat):
				ax.axvline(chosen_kcat, color='black', ls='--', lw=2.0,
					label='chosen kcat')
