"""
Comparison plot for expression of mRNAs and proteins through a polycistronic
operon between simulations with and without operons
"""
...
class Plot(comparisonAnalysisPlot.ComparisonAnalysisPlot):
	def do_plot(self, reference_sim_dir, plotOutDir, plotOutFileName, input_sim_dir, unused, metadata):
		...
		ax2.plot(time_off/60, monomer_counts_off, clip_on=False)
		...
		ax2.set_xlim([0, time_off[-1] / 60])
		ax2.set_ylim([0, 250])
		ax2.set_yticks([0, 250])
		ax2.set_xticks((gen_start_time_off / 60).flatten().tolist() + [time_off.flatten()[-1]/60])
		ax2.set_xticklabels(np.arange(len(gen_start_time_off) + 1))
