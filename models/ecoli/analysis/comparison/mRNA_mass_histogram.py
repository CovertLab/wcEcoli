...
class Plot(comparisonAnalysisPlot.ComparisonAnalysisPlot):
	def do_plot(self, reference_sim_dir, plotOutDir, plotOutFileName, input_sim_dir, unused, metadata):
		...
		ax.hist(
			...
			label=f'reference ({np.mean(mRNA_masses1):.2f} $\pm$ {np.std(mRNA_masses1):.2f}, n={len(mRNA_masses1)})')
		ax.axvline(np.mean(mRNA_masses1), ls='--', lw=2, c='C0')
		ax.hist(
			mRNA_masses2, bins=bins, alpha=0.5,
			label=f'input ({np.mean(mRNA_masses2):.2f} $\pm$ {np.std(mRNA_masses2):.2f}, n={len(mRNA_masses2)})')
