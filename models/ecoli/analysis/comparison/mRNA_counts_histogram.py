...
class Plot(comparisonAnalysisPlot.ComparisonAnalysisPlot):
	def do_plot(self, reference_sim_dir, plotOutDir, plotOutFileName, input_sim_dir, unused, metadata):
		...
		ax.hist(
			...
			label=f'reference ({np.mean(total_mRNA_counts1):.1f} $\pm$ {np.std(total_mRNA_counts1):.1f}, n={len(total_mRNA_counts1)})')
		ax.axvline(np.mean(total_mRNA_counts1), ls='--', lw=2, c='C0')
		ax.hist(
			total_mRNA_counts2, bins=bins, alpha=0.5,
			label=f'input ({np.mean(total_mRNA_counts2):.1f} $\pm$ {np.std(total_mRNA_counts2):.1f}, n={len(total_mRNA_counts2)})')
