"""
Compare histograms of active RNAP counts between two sets of simulations.
"""
...
class Plot(comparisonAnalysisPlot.ComparisonAnalysisPlot):
	def do_plot(self, reference_sim_dir, plotOutDir, plotOutFileName, input_sim_dir, unused, metadata):
		...
		def read_sims(ap):
			...
			# Get index of active RNAPs in the unique molecule counts reader
			...
			unique_molecule_ids = unique_molecule_counts_reader.readAttribute(
				'uniqueMoleculeIds')
			active_RNAP_idx = unique_molecule_ids.index('active_RNAP')

			# Get counts of active RNAP at first timestep from every cell
			active_RNAP_counts = read_stacked_columns(
				cell_paths, 'UniqueMoleculeCounts','uniqueMoleculeCounts',
				ignore_exception=True, fun=lambda x: x[0, active_RNAP_idx])

			return active_RNAP_counts

		# Get initial active RNAP counts from each set of sims
		active_RNAP_counts1 = read_sims(ap1)
		active_RNAP_counts2 = read_sims(ap2)
		...
		ax.set_xlim(BOUNDS)
		ax.set_xlabel('Active RNAP counts')
