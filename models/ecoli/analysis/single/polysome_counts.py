...
class Plot(singleAnalysisPlot.SingleAnalysisPlot):
    def do_plot(self, simOutDir, plotOutDir, plotOutFileName, simDataFile, validationDataFile, metadata):
        ...
        rna_mw = (sim_data.process.transcription.rna_data['mw']/sim_data.constants.n_avogadro).asNumber(units.fg)
        ...
        ribosome_mass = ((ribosome_30S_mass + ribosome_50S_mass)/sim_data.constants.n_avogadro).asNumber(units.fg)
        ...
        polysome_mw_all_time_step = rna_mw[
                                        mRNA_TU_index_flattened_clean] + ribosome_mass * n_ribosome_on_each_mRNA_flattened_clean
        ...
        polysome_count_ax.hist(np.arange(highest_ribosome_count+1), # add 1 to include the highest ribosome count
        ...
        polysome_mw_ax.hist(np.linspace(min(polysome_mw_all_time_step), max(polysome_mw_all_time_step), len(average_polysome_mw)),
