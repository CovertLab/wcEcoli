#!/usr/bin/env python
"""
Analyzes kinetic capacity of tRNA synthetases - calculates kcat fold change required to meet demand.

@author: Heejo Choi, Gwanggyu Sun
	...
	for synthetaseId in synthetaseIds:
		...
		for i, substrate in enumerate(substrateList):						# For each limiting substrate being considered
			...
			if PLOT_VMAX:				# If only the VMAX is being considered
				kcat_needed = aasUsed[:, _aaIndex] / (E * timestep*units.s * cellVolume * nAvogadro)
				ax.plot(time[SKIP_FIRST_N_TIMESTEPS:], np.array(kcat_needed[SKIP_FIRST_N_TIMESTEPS:] / kcat, dtype=float), substrateColor[i])
				# Divide number of tRNAs by number of AAs used each timestep, and plot against time

			else:						# If M-M kinetics is being considered
				if substrate == "tRNA":
					kcat_needed = aasUsed[:, _aaIndex] / (E * timestep*units.s * cellVolume * nAvogadro) * ((T + kM.asUnit(units.mol/units.L)) / T)
				elif substrate == "AA":
					kcat_needed = aasUsed[:, _aaIndex] / (E * timestep*units.s * cellVolume * nAvogadro) * ((AA + kM.asUnit(units.mol/units.L)) / AA)
				else:
					kcat_needed = aasUsed[:, _aaIndex] / (E * timestep*units.s * cellVolume * nAvogadro) * ((atpConcentrations + kM.asUnit(units.mol/units.L)) / atpConcentrations)
