...
class MonomerCounts(wholecell.listeners.listener.Listener):
    ...
    def initialize(self, sim, sim_data):
        ...
        # # Heena's Listeners
        # self.raw_deg_info_dict = sim_data.process.protein_degradation.raw_deg_info_dict
        # self.active_deg_info_dict = sim_data.process.protein_degradation.active_deg_info_dict
    ...
    def allocate(self):
        ...
        self.rawDegradationRate =np.zeros(
            len(self.monomer_ids),
            np.float64
        )

        self.rawDegnProtein = np.zeros(
            len(self.monomer_ids),
            np.int64
        )

        self.activeDegradationRate = np.zeros(
            len(self.monomer_ids),
            np.float64
        )

        self.activeDegnProtein = np.zeros(
            len(self.monomer_ids),
            np.int64
        )
    ...
    def tableCreate(self, tableWriter):
        subcolumns = {
            ...
            'rawDegradationRate': 'monomerIds',
            'rawDegnProtein': 'monomerIds',
            'activeDegradationRate': 'monomerIds',
            'activeDegnProtein': 'monomerIds',
            }
    ...
    def tableAppend(self, tableWriter):
        tableWriter.append(
            ...
            rawDegradationRate = self.rawDegradationRate,
            rawDegradationProtein = self.rawDegnProtein,
            activeDegradationRate=self.activeDegradationRate,
            activeDegnProtein=self.activeDegnProtein,
            )
