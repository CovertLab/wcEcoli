...
Translation elongation sub-model: elongates polypeptides according to the
kinetic constraint of tRNA synthetases.
...
class PolypeptideElongation(wholecell.processes.process.Process):
	...
	def initialize(self, sim, sim_data):
		...
		# Create views onto tRNAs
		isTRna = sim_data.process.transcription.rnaData["isTRna"]
		self.trnas = self.bulkMoleculesView(sim_data.process.transcription.rnaData["id"][isTRna])

		# Create views onto tRNA synthetases
		self.synthetaseIds = sim_data.process.translation.synthetaseKinetics.keys()
		self.synthetases = self.bulkMoleculesView(self.synthetaseIds)

		# Set for timestep calculation
		self.gtpUsed = 0
		self.gtpAvailable = 0

		# Set/Load for elongation
		self.translationSupply = sim._translationSupply
		self.elngRateFactor = 1.
		self.computeSynthetaseKineticCapacity = sim_data.process.translation.computeSynthetaseKineticCapacity
		self.aaIds = sim_data.moleculeGroups.aaIDs
		isTRna = sim_data.process.transcription.rnaData["isTRna"]
		self.trnaIds = sim_data.process.transcription.rnaData["id"][isTRna].tolist()

	def calculateRequest(self):
		# Set ribosome elongation 'rate' (number of expected amino acid
		# incorporations during this time step) with stochasticRound to create
		# single-cell variability in growth rate
		# Default: self.translationSupply is True
		if self.translationSupply:
			# Based on max ribosome elongation rate
			...
			# Based on medium-specific ribosome elongation rate
			self.ribosomeElongationRate = np.min([self.maxRibosomeElongationRate, int(stochasticRound(self.randomState,
				self.elngRateFactor * self.ribosomeElongationRateDict[self.currentNutrients].asNumber(units.aa / units.s)
				* self.timeStepSec()))])
		...
		# Build sequences to estimate amount of amino acids to request
		proteinIndexes, peptideLengths = activeRibosomes.attrs('proteinIndex', 'peptideLength')
		sequences = buildSequences(self.proteinSequences, proteinIndexes, peptideLengths, self.ribosomeElongationRate)
		...
		# Determine number of amino acids to request
		# Default: self.translationSupply is True
		if self.translationSupply:
			# Based on fit supply of amino acids to translation by metabolism
			...
			# Based on estimation from expected elongations and protein seqs
			countAasRequested = aasInSequences

		## KINETIC CAPACITY OF TRNA SYNTHETASES
		# Calculate kinetic capacity of tRNA synthetases
		cellMass = (self.readFromListener("Mass", "cellMass") * units.fg)
		cellVolume = cellMass / self.cellDensity
		countAasCharged = self.computeSynthetaseKineticCapacity(self.aas.total(), self.atp.total(), self.trnas.total(), self.synthetases.total(), self.aaIds, self.trnaIds, self.nAvogadro, cellVolume, self.timeStepSec())
		self.writeToListener("GrowthLimits", "aaRequestedByTranslationSupply", countAasRequested)
		self.writeToListener("GrowthLimits", "aaRequestedBySynthetaseKineticCapacity", countAasCharged)

		countAasRequested = np.fmin(countAasRequested, countAasCharged)

		# Request amino acids
		self.aas.requestIs(countAasRequested.astype(int))
		...
		# GTP hydrolysis is carried out in the Metabolism process for growth
		# associated maintenence. This is set here for metabolism to use
	...
	def evolveState(self):
		...
		# Build sequences for each ribosome to polymerize
		proteinIndexes, peptideLengths, massDiffProtein = activeRibosomes.attrs('proteinIndex', 'peptideLength', 'massDiff_protein')
		sequences = buildSequences(self.proteinSequences, proteinIndexes, peptideLengths, self.ribosomeElongationRate)
		...
		massIncreaseProtein = computeMassIncrease(sequences, sequenceElongations, self.aaWeightsIncorporated)
		...
		activeRibosomes.attrIs(peptideLength = updatedLengths, massDiff_protein = updatedMass)

		# Ribosomes that reach the end of their sequences are terminated and
		# dissociated into 30S and 50S subunits. The polypeptide that they were
		...
		terminatedProteins = np.bincount(proteinIndexes[didTerminate], minlength = self.proteinSequences.shape[0])
		...
		expectedElongations = np.fmin(self.ribosomeElongationRate, terminalLengths - peptideLengths)
