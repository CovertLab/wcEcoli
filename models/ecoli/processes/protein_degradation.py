...
DYNAMIC_PD_TYPE = 2 # 1 is first order, 2 is optimized degradation with beta
    ...
    def calculateRequest(self):
        ...
        if DYNAMIC_PD == True:
            ...
            def get_counts_to_molar():
                ...
                counts_to_molar = (1 / (
                        self.n_avogadro * cell_volume)) # mol/L
            ...
            def get_protease_concentration(protease):
                counts_to_molar = get_counts_to_molar() # mol/L
            ...
            def degrade_proteins(substrates, k_actives, nProteinsToDegrade):
                ...
                for i in range(len(substrate_concentrations)):
                    ...
                    proteins_degraded = np.fmin(
                        self.randomState.poisson(LAMBDA_POSSION), substrate_count
                    )
            ...
            if DYNAMIC_PD_TYPE == 2:
                ...
                self.active_deg_rates = self._proteinDegRates().copy()
                for i, substrate in enumerate(substrates):
                    substrate_idx = np.where(self.protein_IDs == substrate)[0][0]
                    self.active_deg_rates[substrate_idx] = k_actives[i]
                self.active_nProteinsToDegrade = nProteinsToDegrade
