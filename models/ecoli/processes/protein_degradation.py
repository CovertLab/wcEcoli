...
DYNAMIC_PD = True # todo: make this a simulation or ParCa parameter option
	...
	def calculateRequest(self):
		...
		if DYNAMIC_PD == True:
			...
			if DYNAMIC_PD_TYPE == 1:
				...
				# Determine the proteins to degrade based on active degradation rates:
				def degrade_proteins_first_order(substrates, k_actives, nProteinsToDegrade):
					# Obtain the substrate concentrations:
					substrate_concentrations = get_substrate_concentrations(substrates)
					for i in range(len(substrate_concentrations)):
						...
						# Degrade the proteins accordingly:
						nProteinsToDegrade[substrate_idx] = proteins_degraded # todo: check that this is working properly
				...
				nProteinsToDegrade = degrade_proteins_first_order(substrates, k_actives, nProteinsToDegrade)
			...
			if DYNAMIC_PD_TYPE == 2:
				...
				# Determine the proteins to degrade based on active degradation rates:
				# TODO: determine if this function will remain the same every simulation run, and see if just the kactive function can handle the changes so that this can be placed above the method selection.
				def degrade_proteins(substrates, k_actives, nProteinsToDegrade):
					...
					for i in range(len(substrate_concentrations)):
						...
						nProteinsToDegrade[substrate_idx] = proteins_degraded # todo: check to see that this is working
