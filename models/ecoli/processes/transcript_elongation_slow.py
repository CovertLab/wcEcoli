...
class TranscriptElongationSlow(wholecell.processes.process.Process):
	...
	def initialize(self, sim, sim_data):
		super(TranscriptElongationSlow, self).initialize(sim, sim_data)
		...
		# Views
		self.slowRnaBool = ~(sim_data.process.transcription.rnaData["isRRna5S"] |
		...
		self.activeRnaPolys = self.uniqueMoleculesView('activeRnaPoly', rnaIndex = ("in", np.where(self.slowRnaBool)[0]))
