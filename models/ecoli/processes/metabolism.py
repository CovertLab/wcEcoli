...
class Metabolism(wholecell.processes.process.Process):
	...
	def initialize(self, sim, sim_data):
		...
		self.cysE = self.bulkMoleculesView(['SERINE-O-ACETTRAN-MONOMER[c]'])
		self.cysEcplx = self.bulkMoleculesView(['CPLX0-237[c]'])
	...
	def evolveState(self):
		...
		print 'SERINE-O-ACETTRAN-MONOMER[c]', sum(self.cysE.total()), 'CPLX0-237[c]', sum(self.cysEcplx.total())
		...
		enzymeConcentrations[182] = countsToMolar * (enzymeCountsInit[182] + 1)
		...
		# metabolicEnzymes = self.sd.process.metabolism.metabolicEnzymes

		# SEARCH metabolic complexes related to a list of monomeric enzymes (essential or non-essential)
		# essEnzymeCplx = []
		# nonessEnzymeCplx = []
		# for id in xrange(0, len(setessentialenzymes)):
		# 	for monomer in xrange(0, len(moleculeNames)):
		# 		if moleculeNames[monomer] == setessentialenzymes[id]:
		# 			for cplx in xrange(0, len(complexNames)):
		# 				if Stoich[monomer][cplx] != 0:				
		# 					if complexNames[cplx] in metabolicEnzymes:
		# 						essEnzymeCplx.append(complexNames[cplx])
		# for id in xrange(0, len(setnonessentialenzymes)):
	 	# for monomer in xrange(0, len(moleculeNames)):
		# 		if moleculeNames[monomer] == setnonessentialenzymes[id]:
		# 			for cplx in xrange(0, len(complexNames)):
		# 				if Stoich[monomer][cplx] != 0:				
		# 					if complexNames[cplx] in metabolicEnzymes:
		# 						nonessEnzymeCplx.append(complexNames[cplx])
		# setenzymesremoved = self.sd.process.translation.monomerData["id"]
		# for id in xrange(0, len(setenzymesremoved)):
		# 	setStoichCoef = []
		# 	for monomer in xrange(0, len(moleculeNames)):
		# 		if moleculeNames[monomer] == setenzymesremoved[id]:
		# 			for cplx in xrange(0, len(complexNames)):
		# 				if Stoich[monomer][cplx] != 0:				
		# 					if complexNames[cplx] in metabolicEnzymes:
		# 						setStoichCoef.append(Stoich[monomer][cplx])
		# 						# print setenzymesremoved[id], Stoich[monomer][cplx], complexNames[cplx]
		# 	if len(setStoichCoef) == 0: print 0, setenzymesremoved[id]
		# 	if len(setStoichCoef) != 0: print np.abs(np.min(setStoichCoef)), setenzymesremoved[id]

		# setessentialenzymes = []
		# setnonessentialenzymesALL = []
		# setnonessentialenzymes = []			
		# setenzymesremoved = []
		# setenzymes = []
		...
		# if NONZERO_ENZYMES:
			...
			# import ipdb; ipdb.set_trace();
			# PRINT complexes
			# print len(np.unique(essEnzymeCplx))
			# print len(np.unique(nonessEnzymeCplx))
