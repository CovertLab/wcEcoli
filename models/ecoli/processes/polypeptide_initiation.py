...
class PolypeptideInitiation(wholecell.processes.process.Process):
	...
	def evolveState(self):
		...
		# TU_thiMD_prot_idx = self.TU_id_to_protein_index['M007_G7135_RNA[c]']
		#
		...
		#
		# # print(f'TPP_count = {TPP_count}')
		# # print(f'frac_thiCEFSGH_bound = {frac_thiCEFSGH_bound}')
		...
		# 	mRNA_counts[prot_idx] = round(mRNA_counts[prot_idx] * (1-frac_thiCEFSGH_bound))
		# for prot_idx in TU_thiMD_prot_idx:
		# 	mRNA_counts[prot_idx] = round(mRNA_counts[prot_idx] * (1-frac_thiMD_bound))
		#
		...
		for protein_index, counts in zip(
			...
			mask = np.zeros(TU_index_active_mRNAs.shape).astype('bool')

			# if multiple TUs exist for this protein include them in active mRNA
			for idx in self.protein_index_to_TU_index[protein_index]:
				mask[TU_index_active_mRNAs == idx] = True
	...
	def _calculateActivationProb(self, fracActiveRibosome, proteinLengths, ribosomeElongationRates, proteinInitProb,
								 timeStepSec):
		"""
