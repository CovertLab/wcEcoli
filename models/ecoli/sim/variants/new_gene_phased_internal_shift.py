...
- From generations [`NEW_GENE_TRS_INDUCTION_GEN`, `NEW_GENE_TRL_INDUCTION_GEN`]`),
	the new genes will be transcribed using the expression
	factor from the variant index, but not translated.
- From generations [`NEW_GENE_TRL_INDUCTION_GEN`, `NEW_GENE_KNOCKOUT_GEN`),
...
NEW_GENE_TRS_INDUCTION_GEN = 8 # Generation index to induce new gene expression
NEW_GENE_TRL_INDUCTION_GEN = 16 # Generation to induce new gene translation
NEW_GENE_KNOCKOUT_GEN = -1 # Generation index to knock out new gene expression

assert NEW_GENE_TRS_INDUCTION_GEN != 0, (
	"New genes must be induced after the first generation.")
if NEW_GENE_KNOCKOUT_GEN != -1:
	assert NEW_GENE_KNOCKOUT_GEN > NEW_GENE_TRS_INDUCTION_GEN, (
		"New genes are knocked out by default, so induction should happen"
		" before knockout.")
assert NEW_GENE_TRL_INDUCTION_GEN >= NEW_GENE_TRS_INDUCTION_GEN, (
	"If new gene transcripts are not being made, then no translation can occur.")
if NEW_GENE_TRL_INDUCTION_GEN != -1 and NEW_GENE_KNOCKOUT_GEN != -1:
	assert NEW_GENE_TRL_INDUCTION_GEN < NEW_GENE_KNOCKOUT_GEN, (
		"Undefined behavior might occur if the genes are knocked out and then "
		"translation is induced."
	)
...
NEW_GENE_EXPRESSION_FACTORS = [0, 8.5]
NEW_GENE_TRANSLATION_EFFICIENCY_VALUES = [ # Should be descending
    3.40, 0.0]
#	3.40, 3.25, 3.10, 2.95, 2.80,
#	2.65, 2.50, 2.35, 2.20, 2.05,
#	1.90, 1.75, 1.60, 1.45, 1.30,
...
def induce_new_genes_full(sim_data, index):
	"""
...
def induce_new_genes_trs(sim_data, index):
	"""
	'Induces' new genes by setting their expression levels and translation
	effiencies to the values specified using the variant index.
	"""
	...
	# Modify expression only for new genes, ensure translation efficiency is 0
	for i in range(len(new_gene_indices)):
		gene_index = new_gene_indices[i]
		monomer_index = new_gene_monomer_indices[i]
...
def new_gene_phased_internal_shift(sim_data, index):
	"""
	...
	if NEW_GENE_TRS_INDUCTION_GEN != -1:
		sim_data.internal_shift_dict[NEW_GENE_TRS_INDUCTION_GEN] = [
			(induce_new_genes_trs, index_remainder)]
	if NEW_GENE_KNOCKOUT_GEN != -1:
		sim_data.internal_shift_dict[NEW_GENE_KNOCKOUT_GEN] = [
			(knockout_induced_new_gene_expression, index_remainder)]
	if NEW_GENE_TRL_INDUCTION_GEN != -1:
		sim_data.internal_shift_dict[NEW_GENE_TRL_INDUCTION_GEN] = [
			(induce_new_genes_full, index_remainder)]
