...
def initializeProteinMonomers(bulkMolCntr, sim_data, randomState, massCoeff):
	...
	monomerMass = massCoeff * sim_data.mass.getFractionMass(sim_data.conditionToDoublingTime[sim_data.externalState.environment.condition])["proteinMass"] / sim_data.mass.avgCellToInitialCellConvFactor
	...
	monomerExpression = normalize(
		sim_data.process.transcription.rnaExpression[sim_data.externalState.environment.condition][sim_data.relation.rnaIndexToMonomerMapping] *
		sim_data.process.translation.translationEfficienciesByMonomer /
		(np.log(2) / sim_data.conditionToDoublingTime[sim_data.externalState.environment.condition].asNumber(units.s) + sim_data.process.translation.monomerData["degRate"].asNumber(1 / units.s))
		)
...
def initializeRNA(bulkMolCntr, sim_data, randomState, massCoeff):
	...
	rnaMass = massCoeff * sim_data.mass.getFractionMass(sim_data.conditionToDoublingTime[sim_data.externalState.environment.condition])["rnaMass"] / sim_data.mass.avgCellToInitialCellConvFactor
