"""
Extracts protein half-lives from Supplementary Table S1 of Gupta et. al.,
...
# Files
INPUT = os.path.join(FILE_LOCATION, 'protein_half_lives_gupta.tsv')
...
def get_half_lives(symbols_to_monomer_ids):
    ...
    with io.open(INPUT, 'rb') as f:
        ...
        gene_symbol_index = headers.index('Gene name')
        half_life_index = headers.index('MinimalMedia_mean')


        for line in reader:
            gene_symbol = line[gene_symbol_index]
            if line[half_life_index]:
                half_life = float(line[half_life_index]) * 60
