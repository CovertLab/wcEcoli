"""
Extracts some of protein C-limited half-lives from Supplementary Table S1 of Gupta et. al.,
...
OUTPUT_FLAT_FILE = os.path.join(
    ...
    'protein_half_lives_Clim2_TEST_NEW1.tsv')
...
def get_half_lives(symbols_to_monomer_ids):
    ...
    """
    Reads protein half-lives in hours from Gupta et. al. (2022), converts them to minutes,
    ...
    # first, check that the right gene symbol (that the model recognizes) is
    # being used in the input file (and not a synonym)
    original_input = pd.read_csv(INPUT, sep='\t', )
    corrected_input = original_input.copy()
    gene_list = corrected_input['Gene id']

    # load in the rnas file:
    rnas = pd.read_csv(RNAS_FILE, sep='\t', skiprows=[0, 1, 2, 3], )
    common_names = rnas['common_name']
    rnas_synonyms = rnas['synonyms']

    # Check which gene symbols in the input file are not in the common names list:
    genes_in_sort_not_rnas = gene_list[~gene_list.isin(common_names)]
    print('Genes in the sort file that are not in the rnas file:', len(genes_in_sort_not_rnas))

    #

    for gene in genes_in_sort_not_rnas: # Ok so I noticed something very strange.
        # If I loop through every gene, some genes fall into more than one synonym
        # (whereas in my Jupyter notebook, I only looked at genes that were a miss
        # match between the sort file and the common_names in rnas.tsv. NOT sure
        # which is the best way to select the common name if a gene matches 2.
        # And for some reason, when I do it this way (looping through all the options,
        # there are less total matches in the output file! (maybe this is because the duplicates are overriden?))
        for index, synonym in enumerate(rnas_synonyms):
            # Check if the gene is in the current synonyms list
            if gene in synonym:
                # change the value in the sort file to the common name value:
                corrected_input.loc[corrected_input['Gene id'] == gene, 'Gene id'] = \
                    rnas['common_name'][index]
                break  # Stop searching after the first match

    # save the corrected input file (take out the .tsv extension):
    CORRECTED_INPUT = INPUT[:-4] + '_corrected_TEST1.tsv'
    corrected_input.to_csv(CORRECTED_INPUT, sep='\t', index=False)


    id_to_half_life = {}

    with io.open(CORRECTED_INPUT, 'rb') as f:
        reader = tsv.reader(f, delimiter='\t')

        headers = next(reader)

        gene_symbol_index = headers.index('Gene id')
        half_life_index = headers.index('half_life (units.min)')

        for line in reader:
            gene_symbol = line[gene_symbol_index]
            if line[half_life_index]:
                half_life = float(line[
                                      half_life_index])  # TODO: removed a * 60 here. should I have? Yes because the file was already in minutes.
...
def build_half_life_table(raw_half_lives):
    ...
    Args:
        ...
        as reported in Gupta et. al. (2022).
    """
