...
class Equilibrium(object):
	def __init__(self, raw_data, sim_data):
		...
		for reaction in raw_data.equilibrium_reactions:
			...
			for mol_id, coeff in reaction["stoichiometry"].items():
				...
				# Find molecular mass
				molecularMass = sim_data.getter.get_mass(mol_id_with_compartment).asNumber(wholecell.utils.units.g / wholecell.utils.units.mol)
	...
	def __getstate__(self):
		...
		that don't pickle.
		"""
		return wholecell.utils.data.dissoc_strict(self.__dict__, (
			'_stoichMatrix',
	...
	def _populateDerivativeAndJacobian(self):
		...
		self._rates = wholecell.utils.build_ode.rates(self.symbolic_rates)
		self._rates_jacobian = wholecell.utils.build_ode.rates_jacobian(
