	...
	def __init__(self, raw_data, sim_data):
		...
		for subunit in self.subunit_names:
			...
			complex_indicies = np.where(
				...
				(self._stoichMatrixMonomersV < 0))[0] # equivalently, can find the complex indices with (self._stoichMatrixMonomersJ > 0) as the second argument
			...
			for complex_idx in complex_indicies:
				...
				reaction_indicies = np.where(
					(self._stoich_matrix_I == cplx_idx) & (self._stoich_matrix_V > 0))[0]

				# Obtain the value that corresponds to the index of the reaction in self.ids_reactions:
				reaction_idx = self._stoich_matrix_J[reaction_indicies]
				...
				stoich_known['stoich_unknown'] = self.reaction_stoichiometry_unknown[reaction_idx[0]]
				complex_type['complex_type'] = cplx_type
