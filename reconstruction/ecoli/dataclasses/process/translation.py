...
class Translation(object):
    ...
    def _build_translation(self, raw_data, sim_data):
        ...
        max_len = np.int64(
            ...
            + self.max_time_step * sim_data.constants.ribosome_elongation_rate_max.asNumber(
                units.aa / units.s)
        ) + self.next_aa_pad

        self.translation_sequences = np.full((len(sequences), max_len), polymerize.PAD_VALUE,
                                             dtype=np.int8)
        ...
        self.translation_end_weight = (sim_data.getter.get_masses(
            [sim_data.molecule_ids.water]) / sim_data.constants.n_avogadro).asNumber(units.fg)
    ...
    def _build_elongation_rates(self, raw_data, sim_data):
        ...
        self.basal_elongation_rate = sim_data.constants.ribosome_elongation_rate_basal.asNumber(
            units.aa / units.s)
        self.max_elongation_rate = sim_data.constants.ribosome_elongation_rate_max.asNumber(
            units.aa / units.s)
