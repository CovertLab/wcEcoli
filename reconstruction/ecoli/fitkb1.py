...
def fitKb_1(kb):
	...
	#		(2*np.exp(-np.log(2)*kb.process.transcription.rnaData["ageReplicated"]/kb.doubling_time)/(kb.process.transcription.rnaData["degRate"]+np.log(2)/kb.doubling_time))

	##################################
	### Vary gene copy number code ###

	#This is like single cell time average expression -- I think it makes more sense to fit this
	expressionToFit = kb.process.transcription.rnaData["synthProbTimeAvg"] / (kb.process.transcription.rnaData["degRate"].asNumber(1 / units.s)+np.log(2)/kb.doubling_time.asNumber(units.s))
	...
	kb.process.transcription.rnaData["expressionInitial"] = kb.process.transcription.rnaData["synthProb"]/(kb.process.transcription.rnaData["degRate"].asNumber(1 / units.s)+np.log(2)/kb.doubling_time.asNumber(units.s))
