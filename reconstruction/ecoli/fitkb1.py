...
def fitKb_1(kb):
	...
	#expressionToFit = kb.process.transcription.rnaData["synthProbTimeAvg"] * 
	#		(2*np.exp(-np.log(2)*kb.process.transcription.rnaData["ageReplicated"]/kb.cellCycleLen)/(kb.process.transcription.rnaData["degRate"]+np.log(2)/kb.cellCycleLen))

	##################################
	### Vary gene copy number code ###

	#This is like single cell time average expression -- I think it makes more sense to fit this
	expressionToFit = kb.process.transcription.rnaData["synthProbTimeAvg"] / (kb.process.transcription.rnaData["degRate"].asNumber(1 / units.s)+np.log(2)/kb.constants.cellCycleLen.asNumber(units.s))
	expressionToFit /= expressionToFit.sum()
	kb.process.transcription.rnaData["expression"] = expressionToFit
	...
	synthProbFromFitTimeAvgs = np.zeros(len(kb.process.transcription.rnaData["synthProbTimeAvg"]))
	synthProbFromFitTimeAvgs[kb.process.transcription.rnaData["isHighlyRegulated"]] = kb.process.transcription.rnaData["synthProbTimeAvg"][kb.process.transcription.rnaData["isHighlyRegulated"]]
	fractionSynthProbNotHighlyRegulated = 1 - np.sum(synthProbFromFitTimeAvgs[kb.process.transcription.rnaData["isHighlyRegulated"]])
	synthProbFromFitTimeAvgs[~kb.process.transcription.rnaData["isHighlyRegulated"]] = kb.process.transcription.rnaData["synthProbTimeAvg"][~kb.process.transcription.rnaData["isHighlyRegulated"]]/timeAvgProbAsFuncofCopyAge(kb.process.transcription.rnaData["ageReplicated"][~kb.process.transcription.rnaData["isHighlyRegulated"]].asNumber())
	#synthProbFromFitTimeAvgs = kb.process.transcription.rnaData["synthProbTimeAvg"]/timeAvgProbAsFuncofCopyAge(kb.process.transcription.rnaData["ageReplicated"].asNumber())
	synthProbFromFitTimeAvgs[~kb.process.transcription.rnaData["isHighlyRegulated"]] = synthProbFromFitTimeAvgs[~kb.process.transcription.rnaData["isHighlyRegulated"]]/np.sum(synthProbFromFitTimeAvgs[~kb.process.transcription.rnaData["isHighlyRegulated"]]) * (1-np.sum(synthProbFromFitTimeAvgs[kb.process.transcription.rnaData["isHighlyRegulated"]]))
	#synthProbFromFitTimeAvgs /= synthProbFromFitTimeAvgs.sum()
	kb.process.transcription.rnaData["synthProb"] = synthProbFromFitTimeAvgs
	kb.process.transcription.rnaData["expressionInitial"] = kb.process.transcription.rnaData["synthProb"]/(kb.process.transcription.rnaData["degRate"].asNumber(1 / units.s)+np.log(2)/kb.constants.cellCycleLen.asNumber(units.s))
	kb.process.transcription.rnaData["expressionInitial"] /= np.sum(kb.process.transcription.rnaData["expressionInitial"])
...
def fitExpression(kb, bulkContainer):
	...
	kb.process.transcription.rnaData["synthProbTimeAvg"][:] = synthProbTimeAvg


def fitRNAPolyTransitionRates(kb):
	## Transcription activation rate

	synthProb = kb.process.transcription.rnaData["synthProbTimeAvg"]
